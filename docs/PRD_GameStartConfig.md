...
- 设备映射区：展示 `requiredDevices`（`logicalId`、能力、是否必需），选择满足能力要求的在线设备，显示连接状态与能力匹配提示。
...
## 交互与校验
- 设备映射：所有 `required=true` 的逻辑设备必须映射到一个在线设备；设备必须同时满足该逻辑设备声明的全部能力。
...
- `GET /api/device-types`：设备类型映射（用于展示设备类型名称）。
- `GET /api/device-capabilities`：设备能力目录与 `typeCapabilityMap`（用于过滤与推荐）。
...
- `GET /api/games/:id/meta`：返回玩法元信息：
  - `requiredDevices: Array<{ logicalId, capabilities: string[], required: boolean, description? }>`
