...
| 通道 | 调什么 | 用什么 |
|------|--------|--------|
| **CDP**（Chrome DevTools Protocol） | 渲染进程：Vue 前端页面、`<webview>` 内置浏览器。控制台 / 网络 / DOM / 执行 JS | 远程调试端口 `9222` |
...
验证端口开了：浏览器打开 `http://127.0.0.1:9222/json`，
...
`127.0.0.1:9222` → 下方 `Remote Target` 会列出 electron 里的页面 →
点 `inspect` 就弹出一个完整的 DevTools 面板，和平时按 F12 一模一样。

**方式 B — 让 AI（Kiro）用 chrome-devtools 工具连（自动调）**
需要把 `chrome-devtools` MCP 配成连接模式，指向 9222。
详见文末「附录：让 AI 连上 9222」。连上后 AI 能读控制台、抓网络请求、
...
[
  ...
  "--browserUrl=http://127.0.0.1:9222"   // 加这行；连接模式下可去掉 userDataDir
]
```

步骤：
1. 先 `npm run electron:debug` 把 electron 和 9222 端口起起来；
