...
const pressureEdging = {
  ...
  parameter: [
    ...
    { key: 'midPressure', type: 'number', name: '强度减弱气压点', required: false, default: 19.2, min: 0, max: 60, step: 0.1 },
  ],
  ...
  loop(deviceManager) {
    ...
    */
    ...
    if(pressure >= cfg.criticalPressure){
      ...
      //处于延迟期，压力已回落，判断是否处于（等待压力回落）状态，若是，更改此状态为假，发送文本
      if(this._runtime.waitPressureDrop){
          ...
          this._runtime.waitPressureDrop = false;
        }
    }else if(pressure <= cfg.midPressure){
      const increaseIntensity = dtSec * (cfg.intensityGradualIncrease || 0);
      this._runtime.targetIntensity += increaseIntensity;
      if(this._runtime.targetIntensity >= cfg.maxMotorIntensity)
        this._runtime.targetIntensity = cfg.maxMotorIntensity;
      this._runtime.midIntensity = this._runtime.currentIntensity;
    }else{
      let intensity = this._runtime.midIntensity*((cfg.criticalPressure-pressure)||0)/Math.max(0.01,(cfg.criticalPressure-cfg.midPressure));
    ...
    if((now-this._runtime.delayStartTime)/1000>=cfg.lowPressureDelay){
      if(this._runtime.isInDelayPeriod === true){ 
        ...
        //之前处于延迟期，等待时间超过延迟时间，计算初始强度，发送文本
        const pressureDiff = cfg.criticalPressure - pressure;
        const normalizedDiff = pressureDiff / Math.max(1e-6, (cfg.criticalPressure-cfg.pressureSensitivity));
        const baseTarget = normalizedDiff * cfg.maxMotorIntensity;
      ...
      if(this._runtime.isInDelayPeriod === false){
        ...
        //之前处于非延迟期，超过临界压力，电击，显示信息，寸止次数加一，（等待压力回落）状态为真，发送文本
        this._triggerShock(deviceManager);
        deviceManager.log('info', `超过临界压力，强度设置为0`);
        deviceManager.emitUi({ fields: { statusText: '待压力降低…' } });
        this._runtime.totaldeniedTimes ++;
        this._runtime.waitPressureDrop = true;
      }
